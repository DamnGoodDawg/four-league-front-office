...
## D17 — D1 write discipline: diff-persistence, change-only logging (2026-09-14)
The delete-and-reinsert sync burned Cloudflare D1's free daily cap (100k rows_written; deletes and index updates count) during the first Sunday game window — Cloudflare emailed at 96%. Fix: persistence reads current rows (reads are budgeted ~50x higher) and writes only actual changes; sync_log records status changes only, never steady-state "ok"s; overnight cadence relaxed to 30 min. Result: a quiet tick writes 0 rows, a live-game tick writes only players whose numbers moved (~10-50), estimated <10k rows/day vs ~150k+ before. Every sync reports "wrote N" for observability. Workers Paid ($5/mo, 50M writes) remains the escape hatch if ever needed — not currently.
